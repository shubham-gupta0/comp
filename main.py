...
def compare_images(image1_file, image2_file):
    ...
    return result, float(distance)  # Convert numpy.float32 to native Python float
...
class CompareMuzzlesResponse(BaseModel):
    result: str
    distance: float

@app.post("/compare-muzzles", response_model=CompareMuzzlesResponse)
async def compare_muzzles(file1: UploadFile = File(...), file2: UploadFile = File(...)):
    try:
        ...
        return CompareMuzzlesResponse(result=result, distance=distance)
    except Exception as e:
